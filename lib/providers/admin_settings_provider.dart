...
class AdminSettingsProvider with ChangeNotifier {
  ...
  bool _getBoolValue(String key, {bool defaultValue = false}) {
    ...
    if (setting?.value is bool) return setting!.value;
    if (setting?.value is String) return setting!.value.toString().toLowerCase() == 'true';
    return defaultValue;
  }

  int _getIntValue(String key, {int defaultValue = 1}) {
    final setting = _settings[key];
    if (setting?.value is int) return setting!.value;
    if (setting?.value is String) return int.tryParse(setting!.value) ?? defaultValue;
    return defaultValue;
  }
